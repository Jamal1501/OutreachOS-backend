...
class TaskQueueService
{
    ...
    private function candidateArray(
        ...
        $decision = $this->buildTaskDecisionMetadata($profile, $taskType, $priority, $dueAt, $actionableChannel, $metadata);
        $metadata = array_merge($metadata, $decision);
        ...
        return [
            ...
            'notes' => $notes,
            'metadata' => $metadata,
            'rank_score' => (int) ($decision['decision_score'] ?? 0),
        ];
    }

    private function buildTaskDecisionMetadata(
        CreatorProfile $profile,
        string $taskType,
        string $priority,
        Carbon $dueAt,
    ...
    private function normalizeDbTask(Task $task): array
    {
        ...
        $groupMeta = $this->groupMetaForTask((string) $task->task_type, $metadata);
        if (!isset($metadata['decision_score']) && $task->creatorProfile instanceof CreatorProfile) {
            $metadata = array_merge($metadata, $this->buildTaskDecisionMetadata(
                $task->creatorProfile,
                (string) $task->task_type,
                (string) ($task->priority ?: 'MEDIUM'),
                $task->due_at instanceof Carbon ? $task->due_at : now(),
                (string) ($task->actionable_channel ?: $task->platform ?: ''),
                $metadata
            ));
        }
        ...
        return [
            ...
            'decisionScore' => (int) ($metadata['decision_score'] ?? 0),
            'decisionReason' => (string) ($metadata['decision_reason'] ?? ''),
            'whyNow' => (string) ($metadata['why_now'] ?? ''),
            'successCondition' => (string) ($metadata['success_condition'] ?? ''),
            'fallbackAction' => (string) ($metadata['fallback_action'] ?? ''),
            'decisionConfidence' => (int) ($metadata['decision_confidence'] ?? 0),
